...
const COPY = {
  en: {
    ...
    polls: "Family polls", maintenance: "Home maintenance", meals: "Weekly menu", school: "School", pantry: "Pantry & household stock", routines: "Family routines", calendar: "Family calendar", today: "Family today", shopping: "Shopping", tasks: "Tasks", court: "Rules & rewards",
  ...
  ru: {
    ...
    polls: "Семейные голосования", maintenance: "Обслуживание дома", meals: "Меню на неделю", school: "Школа", pantry: "Продукты и запасы", routines: "Семейные рутины", calendar: "Семейный календарь", today: "Семья сегодня", shopping: "Покупки", tasks: "Задачи", court: "Правила и поощрения",
  ...
  uk: {
    ...
    polls: "Родинні голосування", maintenance: "Обслуговування дому", meals: "Меню на тиждень", school: "Школа", pantry: "Продукти й запаси", routines: "Сімейні рутини", calendar: "Сімейний календар", today: "Родина сьогодні", shopping: "Покупки", tasks: "Завдання", court: "Правила та заохочення",
...
export class FamilyCard extends HTMLElement {
  ...
  setConfig(config) {
    ...
    if (!["today","shopping","tasks","court","alarms","health","conversation","mikrotik","calendar","routines","pantry","meals","school","maintenance","polls"].includes(this._view)) throw new Error("Unknown Family Assistant view");
    ...
    this._dietaryDraft=null;this._recipesDraft=null;this._schoolDraft=null;this._maintenanceDraft=null;this._schoolWorkDraft=null;this._schoolReminderDraft=null;this._pollsDraft=null;
  ...
  async refresh() {
    ...
    try {
      ...
      if (dietaryForce || recipesForce || schoolForce || schoolWorkForce || schoolReminderForce || maintenanceForce || pollsForce || mediaForce || !this.shadowRoot.activeElement?.closest("form")) this.render();
...
class FamilyEditor extends HTMLElement {
  ...
  render(){
    ...
    for(const [name,label] of [["entry_id",t.entry],["title",t.name],["view",t.view]]){
      ...
      if(name==="view")for(const view of ["today","shopping","tasks","court","alarms","health","conversation","mikrotik","calendar","routines","pantry","meals","school","maintenance","polls"]){const option=el("option",t[view]);option.value=view;input.append(option);}
      const defaultView={"custom:family-alarms-card":"alarms","custom:family-shopping-card":"shopping","custom:family-tasks-card":"tasks","custom:family-court-card":"court","custom:family-health-card":"health","custom:family-conversation-card":"conversation","custom:family-network-card":"mikrotik","custom:family-calendar-card":"calendar","custom:family-routines-card":"routines","custom:family-pantry-card":"pantry","custom:family-meals-card":"meals","custom:family-school-card":"school","custom:family-maintenance-card":"maintenance","custom:family-polls-card":"polls"}[this._config?.type] || "today";
...
for(const [type,view] of [["family-assistant-card","today"],["family-shopping-card","shopping"],["family-tasks-card","tasks"],["family-court-card","court"],["family-alarms-card","alarms"],["family-health-card","health"],["family-conversation-card","conversation"],["family-network-card","mikrotik"],["family-calendar-card","calendar"],["family-routines-card","routines"],["family-pantry-card","pantry"],["family-meals-card","meals"],["family-school-card","school"],["family-maintenance-card","maintenance"],["family-polls-card","polls"]]){
