...
export function renderKids(card,body){
  ...
  for(const p of data.profiles){
    ...
    const tz=card._data.settings?.timezone || "UTC";
    const st=p.status;
    const validUntilMs=st?.valid_until ? Date.parse(st.valid_until) : NaN;
    const isKnown = Boolean(
      st &&
      typeof st.allows === "boolean" &&
      ["schedule","paused","unrestricted"].includes(st.mode) &&
      Number.isFinite(validUntilMs) &&
      validUntilMs > Date.now()
    );
    if(!isKnown){
      section.append(el("p","⚠️ "+t.statusUnknown,"notice"));
    }else{
      section.append(el("p",modeLabels[st.mode]));
      section.append(el("p",st.allows?t.statusAllowed:t.statusBlocked));
      if(st.allows && st.remaining_minutes!=null){
        section.append(el("p",`${t.remaining}: ${st.remaining_minutes} ${t.minUnit}`,"sub"));
      }
      if(st.next_change_at){
        const nextLabel=st.next_allows?t.nextAllowed:t.nextBlocked;
        const nextTimeStr=new Date(st.next_change_at).toLocaleString(lang,{timeZone:tz});
        section.append(el("p",`${nextLabel}: ${nextTimeStr} (${tz})`,"sub"));
      }
      if(st.temporary_until){
        const tempLabel=st.temporary_mode==="timed_pause"?t.temporaryPauseExpiry:t.temporaryExpiry;
        const tempTimeStr=new Date(st.temporary_until).toLocaleString(lang,{timeZone:tz});
        section.append(el("p",`${tempLabel}: ${tempTimeStr} (${tz})`,"sub"));
      }
    }
    if(!p.observed){section.append(el("p",t.stale,"notice"));root.append(section);continue;}
    const schedule=el("details");schedule.append(el("summary",t.weeklySchedule));
