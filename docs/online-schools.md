...
The original branch was independent of the household control-center work.
The rc.5 integration merge below combines its source with the current release;
subsequent control-center changes require another combined acceptance run.
...
## Integration with rc.5 (2026-09-13, local checkpoint)

`codex/school-release-integration` merges school tip
`b7bd2b42d0558e1c081892be9deb1720c2e7c9ed` into release source
`bfc27097` (0.2.0-rc.5); common ancestor `6dd783809309cbfd3c612b268a42b57642f8df28`
was rc.1. The 13 conflicted files were reconciled by retaining the current
ordered provider/image settings, grounded search and family controls alongside
school code. Both image and school workers stop on successful unload; a failed
platform unload leaves both running. Shared reply delivery independently checks
image, school, actor and private-chat scope. School Options preserve ordered
text/image/Telegram configuration and do not expose their secrets in forms.

Thirteen new composition cases exercise those boundaries, including the actual
Telegram manager with both provider types configured, deterministic school reads,
and an ordinary model request replying to a retained school message. The school
body is excluded from that job. No real provider, bot or household was contacted.

Local evidence: the initial full Python suite passed 6,099 tests, skipped six,
and passed 23 subtests; its only failure identified three absent school rows in
the native settings inventory. Those rows are now documented, and the subsequent
238-case focused run passed including the inventory and all new composition
cases. Ruff, formatting, locale parity, public-tree privacy and runtime ZIP/import
checks passed. The combined runtime package had 317 files; this local build is
not a published release. Eight school Chromium cases passed in EN/RU/UK at mobile
and desktop sizes, with privacy, inert HTML and source-revocation checks.

The first JavaScript run passed 575 main tests and failed the two control-inventory
gates: source hashes changed and the incoming browser suite lacked the current
audit adapter. The suite now uses that adapter and school assets are classified
under School. The durable inventory deliberately remains at its previous evidence
until the coordinator merges the remaining control-center changes and runs the
full synthetic action audit. An ordinary full browser run was interrupted for
that handoff; it is not reported as a full pass. The attempted bounded AGY review
returned no review text and is not verification evidence.

Release gates still required on the final combined commit: full Python/JavaScript,
full audited Chromium inventory regeneration, and isolated HA 2026.9.2 `all`,
`settings`, `panel` and `online-school` cases through `ha_dependency_smoke.py`.
The CI school case now uses the existing 2026.8.2/2026.9.2 matrix alongside the
settings case. Its original-branch HA 2026.8.2 result above does not establish a
pass on this merged source. Current provider chains, image settings, Telegram
reply scopes and school Options/lifecycle must all pass together before release.
No production deployment or permission for school submissions is implied.
