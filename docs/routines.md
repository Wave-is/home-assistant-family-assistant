...
* **Current UI Scope**: Recurrence, per-step assignees, template/step skip conditions and per-step completion conditions are editable in the card. The editor offers household modes, approved entity states, local time windows, negation and nested all/any groups (depth 3, 20 total nodes). Template skip applies only at creation; step skip only at activation; unknown never skips or completes. Entity completion must reference at least one approved entity. Use “No condition” to clear explicitly. Switching to manual completion removes its condition on save, but keeps the unsaved draft if switched back. Reordering/handoff preserves each rule. Existing groups survive metadata edits and all/any changes. Unsupported future fields require explicit replacement; they are never silently removed. Observation permissions, member revisions and source revisions are checked again. An uncertain save/start keeps its own exact retry identity across other commands.
...
* **Правки и UI**: Правка шаблона не перезаписывает текущие выполнения. `enabled: false` запрещает новые старты. В карточке настраиваются повторения, исполнители и условия пропуска/завершения отдельных шагов: режим дома, состояние разрешённого объекта, временной интервал, отрицание и группы «все/любое» (до 3 уровней и 20 условий). Пропуск всего распорядка проверяется при старте, шага — при его активации; неизвестное условие не пропускает и не завершает шаг. Автозавершение требует хотя бы один разрешённый объект. «Без условия» явно удаляет правило. Ручное завершение удаляет условие при сохранении, но переключение обратно сохраняет несохранённый черновик. Перестановка и передача шага сохраняют его правило. Неизвестные поля будущих версий требуют явной замены. При сохранении заново проверяются разрешения и версии записи/участников. Неопределённый результат сохранения или старта сохраняет собственный точный повтор команды.
...
* **Зміни та стан UI**: Зміна шаблону не змінює активні виконання. `enabled: false` зупиняє нові старти. У картці налаштовуються повторення, виконавці та умови пропуску/завершення кроків: режим дому, стан дозволеного об’єкта, часовий інтервал, заперечення та групи «усі/будь-яка» (до 3 рівнів і 20 умов). Пропуск усього розпорядку перевіряється під час запуску, кроку — під час активації; невідома умова не пропускає й не завершує крок. Автозавершення потребує хоча б одного дозволеного об’єкта. «Без умови» явно видаляє правило. Ручне завершення видаляє умову під час збереження, але перемикання назад зберігає незбережену чернетку. Переставлення та передавання кроку зберігають його правило. Невідомі поля майбутніх версій потребують явної заміни. Під час збереження знову перевіряються дозволи й версії запису/учасників. Невизначений результат збереження або запуску залишає власний точний повтор команди.
