...
test("parent adds an item using the Russian mobile card",async({page})=>{
 ...
 expect(await page.evaluate(()=>window.calls.length)).toBe(0);
 await page.getByRole("button",{name:"Добавить в список покупок",exact:true}).click();
 await expect(page.getByText("Молоко",{exact:true})).toBeVisible();
 expect(await page.evaluate(()=>window.calls[0].payload)).toEqual({name:"Молоко",quantity:2,unit:"л",category:"",store:"",note:"",buyer:null});
 expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);
