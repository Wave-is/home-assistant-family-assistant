...
async def test_school_edit_preserves_current_ordered_ai_image_and_telegram_options(harness):
    shared = {
        "conversation": {
            "enabled": True,
            "providers": [
                {"id": "text", "kind": "agy", "api_key": OTHER_SECRET},
                {"id": "fallback", "kind": "ollama", "model": "synthetic-model"},
            ],
            "search": {"enabled": True, "provider": "agy"},
        },
        "image_generation": {
            "enabled": True,
            "providers": [{"id": "image", "type": "comfyui", "api_key": OTHER_SECRET}],
        },
        "telegram": {"bots": [{"id": 123456, "token": OTHER_SECRET}]},
    }
    harness.entry.options.update(deepcopy(shared))
    account = await begin(harness)
    student = await harness.flow.async_step_online_school_account(account_values())
    assert all(OTHER_SECRET not in visible_form(form) for form in (account, student))
    result = await harness.flow.async_step_online_school_student(student_values())
    assert result["type"] == "create_entry"
    for key, value in shared.items():
        assert result["data"][key] == value
    assert "provider_chain" in (await harness.flow.async_step_menu_ai())["menu_options"]
    assert "image_generation" in (await harness.flow.async_step_menu_ai())["menu_options"]
    assert "online_school" in (await harness.flow.async_step_menu_services())["menu_options"]
