...
async def test_unload_preserves_or_stops_both_school_and_image_workers_together(
    runtime_adapter, engine, monkeypatch, platforms_unload
):
    adapter, _workers = runtime_adapter
    calls = []

    async def unload_platforms(entry, platforms):
        return platforms_unload

    async def stop_school():
        calls.append("school")

    runtime = adapter.Runtime(engine)
    runtime.image_generation = SimpleNamespace(close=lambda: calls.append("images"))
    runtime.online_school = SimpleNamespace(stop=stop_school)
    entry = SimpleNamespace(entry_id="synthetic-composition", runtime_data=runtime)
    hass = SimpleNamespace(
    ...
    frontend.DATA_PANELS = "panels"
    frontend.async_remove_panel = lambda *args: None
    monkeypatch.setitem(sys.modules, frontend.__name__, frontend)
    before = engine.snapshot()
    assert await adapter._async_unload_runtime(hass, entry) is platforms_unload
    assert calls == (["images", "school"] if platforms_unload else [])
    assert engine.snapshot() == before
    assert (entry.entry_id in hass.data["family_assistant"]["entries"]) is not platforms_unload
