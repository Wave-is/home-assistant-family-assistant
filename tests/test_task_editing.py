...
async def test_unchanged_reassignment_or_instant_does_not_reset_progress(engine, now):
    ...
    started = await engine.execute(
        "child", "tasks.start", {"id": item["id"], "revision": item["revision"]}, "start", now
    )
...
async def test_reassignment_preserves_report_and_supersedes_old_private_assignment(engine, now):
    ...
    await engine.execute(
        "child",
        "tasks.submit",
        {"id": item["id"], "revision": item["revision"], "report": "Old report"},
        "report",
        now,
    )
    await engine.execute(
...
async def test_action_specific_fields_prevent_silently_ignored_mutations(engine, now):
    ...
    with pytest.raises(DomainError, match="invalid_field"):
        await engine.execute(
            "parent",
            "tasks.complete",
            {"id": item["id"], "revision": item["revision"], "due_at": None},
            "wrong-action",
            now,
        )
    assert engine.snapshot() == before
...
async def test_guest_assignment_and_duplicate_archive_rejected(engine, now):
    ...
    with pytest.raises(DomainError, match="invalid_field"):
        await engine.execute(
            "parent",
            "tasks.revise",
            {"id": item["id"], "revision": item["revision"], "assignee": "guest"},
            "guest",
            now,
        )
    archived = await engine.execute(
        "parent", "tasks.archive", {"id": item["id"], "revision": item["revision"]}, "archive", now
    )
    before = engine.snapshot()
